...
import Package.model.Tabuleiro;
import Package.model.Space;
...
import static Package.util.ModeloTabuleiro.MODELO_TABULEIRO;
...
public class Main {
    ...
    for (int i = 0; i < LIMITE_TABULEIRO; i++) {
        ...
        for (int j = 0; j < LIMITE_TABULEIRO; j++) {
            ...
            // Verificação de nulidade adicionada aqui
            if (isNull(positionConfig)) {
                System.err.printf("Atenção: Configuração para a posição [%d,%d] não encontrada. O jogo pode não ser iniciado corretamente.%n", i, j);
    ...
    private static void inputNumber() {
        ...
        if (!tabuleiro.changeValue(col, row, value)){
